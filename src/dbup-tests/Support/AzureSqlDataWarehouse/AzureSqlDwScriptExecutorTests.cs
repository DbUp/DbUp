...
{
    public class AzureSqlDwScriptExecutorTests
    {
        [Fact]
        public void verify_schema_should_not_check_when_schema_is_null()
        {
            var executor = new AzureSqlDwScriptExecutor(() => Substitute.For<IConnectionManager>(), () => null, null, () => false, null, () => Substitute.For<IJournal>());
        ...
        public void when_schema_is_null_schema_is_stripped_from_scripts()
        {
            ...
            var executor = new AzureSqlDwScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => new ConsoleUpgradeLog(), null, () => true, null, () => Substitute.For<IJournal>());
        ...
        public void does_not_use_variable_subtitute_preprocessor_when_setting_false()
        {
            ...
            var command = Substitute.For<IDbCommand>();
            dbConnection.CreateCommand().Returns(command);
            var executor = new AzureSqlDwScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => new ConsoleUpgradeLog(), null, () => false, null, () => Substitute.For<IJournal>());
        ...
        public void uses_variable_subtitutes_schema()
        {
            ...
            var command = Substitute.For<IDbCommand>();
            dbConnection.CreateCommand().Returns(command);
            var executor = new AzureSqlDwScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => new ConsoleUpgradeLog(), "foo", () => true, null, () => Substitute.For<IJournal>());
        ...
        public void logs_output_when_configured_to()
        {
            ...
            var command = Substitute.For<IDbCommand>();
            dbConnection.CreateCommand().Returns(command);
            var executor = new AzureSqlDwScriptExecutor(() => new TestConnectionManager(dbConnection, true)
            {
        ...
        public void logs_when_dbexception()
        {
            ...
            var executor = new AzureSqlDwScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => logger, null, () => true, null, () => Substitute.For<IJournal>());
