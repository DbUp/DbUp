...
namespace DbUp.Tests.Support.SqlServer
{
    ...
    public class SqlScriptExecutorTests
    {
        ...
        {
            var executor = new ScriptExecutor(() => Substitute.For<IConnectionManager>(), () => null, null, () => false, null, new SqlObjectParser(), new SqlCreateSchema());
        ...
        public void when_schema_is_null_schema_is_stripped_from_scripts()
        {
            ...
            var executor = new ScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => new ConsoleUpgradeLog(), null, () => true, null, new SqlObjectParser(), new SqlCreateSchema());
        ...
        public void does_not_use_variable_subtitute_preprocessor_when_setting_false()
        {
            ...
            var command = Substitute.For<IDbCommand>();
            dbConnection.CreateCommand().Returns(command);
            var executor = new ScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => new ConsoleUpgradeLog(), null, () => false, null, new SqlObjectParser(), new SqlCreateSchema());
        ...
        public void uses_variable_subtitutes_schema()
        {
            ...
            var command = Substitute.For<IDbCommand>();
            dbConnection.CreateCommand().Returns(command);
            var executor = new ScriptExecutor(() => new TestConnectionManager(dbConnection, true), () => new ConsoleUpgradeLog(), "foo", () => true, null, new SqlObjectParser(), new SqlCreateSchema());
        ...
        public void logs_output_when_configured_to()
        {
            ...
            var command = Substitute.For<IDbCommand>();
            dbConnection.CreateCommand().Returns(command);
            var executor = new ScriptExecutor(() => new TestConnectionManager(dbConnection, true)
            {
                IsScriptOutputLogged = true
            }, () => new ConsoleUpgradeLog(), "foo", () => true, null, new SqlObjectParser(), new SqlCreateSchema());
